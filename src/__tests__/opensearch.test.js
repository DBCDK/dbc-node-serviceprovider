...
describe('Test transform of OpenSearch responses', () => {
	it('Check one hit', function() {
		...
		assert.equal(JSON.stringify(transform.responseTransform(response)), JSON.stringify({"result":[{"identifiers":["870970-basis:27036031"],"title":"Harry Potter og Fønixordenen","workType":"game"}],"error":[]}), "One hit in search result");
	...
	it('Check more hits', function() {
		...
		assert.equal(JSON.stringify(transform.responseTransform(response)), JSON.stringify({"result":[{"identifiers":["870970-basis:29317038"],"title":"Harry Potter og De Vises Sten","workType":"book"},{"identifiers":["870970-basis:29317038"],"title":"Harry Potter og De Vises Sten","workType":"book"}],"error":[]}), "More hits in search result");
	...
	it('Check more hits one work', function() {
		...
		assert.equal(JSON.stringify(transform.responseTransform(response)), JSON.stringify({"result":[{"identifiers":["870970-basis:22252852","870970-basis:29317038"],"title":"Harry Potter og De Vises Sten","workType":"book"}],"error":[]}), "More hits in one work collection");
	...
	it('Check no hits', function() {
		...
		assert.equal(JSON.stringify(transform.responseTransform(response)), JSON.stringify({"result":[{"hits":"0","collections":"0","more":"false"}],"error":[]}), "No records found");
	...
	it('Check authentication error', function() {
		...
		assert.equal(JSON.stringify(transform.responseTransform(response)), JSON.stringify({"result":[],"error":[{"errorcode":1,"errormessage":"Authentication error","serviceerror":"authentication_error"}]}), "Authentication error");
